...
<form class="panel form-panel" method="post" action="<?= base_url('admin/sermoes/salvar') ?>">
  <?= Csrf::field() ?>
  ...
  <label>Conteúdo * (aceita HTML: &lt;p&gt;, &lt;h2&gt;, &lt;strong&gt;, &lt;ul&gt;…)<textarea name="content" rows="18" required><?= e($item['content'] ?? '') ?></textarea></label>

  <div class="form-row">
    <label>Vídeo (URL do YouTube — opcional)<input type="text" name="video_url" value="<?= e($item['video_url'] ?? '') ?>"></label>
    <label>Imagem de destaque (URL)<input type="text" name="image" value="<?= e($item['image'] ?? '') ?>" data-upload-target></label>
  </div>
  ...
  <div class="form-actions">
    <a class="btn btn-outline" href="<?= base_url('admin/sermoes') ?>">Cancelar</a>
    <button class="btn btn-primary" type="submit">Salvar sermão</button>
  </div>
</form>
